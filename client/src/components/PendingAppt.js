  ...
  return (
    <div>
      ...
      {currentUser.hairstyle_pic ? (
        <button name='Reject' onClick={RejectAppt}>Delete Appointment Request</button>
      ) : (
        <>
          <button name='Accept' onClick={AcceptAppt}>Accept Appointment</button>
          <button name='Reject' onClick={RejectAppt}>Reject Appointment</button>
        </>
      )}
    </div>
  );
